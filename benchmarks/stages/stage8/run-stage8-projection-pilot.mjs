...
const protocolFile = path.join(repositoryRoot, "docs/stage8/projection-live-protocol.json")
...
if (!protocol.tasks.includes(id)) throw new Error("usage: node benchmarks/stages/stage8/run-stage8-projection-pilot.mjs --id <protocol-task-id> [--run]")
...
const mapFiles = ["benchmarks/stages/stage8/map/project-v3.ts", "benchmarks/stages/stage8/map/inject.mjs", "plugins/token-efficient/lib/context/extract.ts", "plugins/token-efficient/lib/context/retrieve.ts", "plugins/token-efficient/lib/context/config.ts"]
const mapCodeHash = sha(mapFiles.map(f => `${f}\0${sha(fs.readFileSync(path.join(root, f)))}\n`).join(""))
...
const projected = spawnSync("bun", [path.join(here, "map/project-v3.ts"), workspace, promptFile], { cwd: root, encoding: "utf8", timeout: 15000 })
if (projected.status !== 0) throw new Error(`projection failed: ${projected.stderr}`)
const mapped = JSON.parse(projected.stdout)
const mapPrepMs = Date.now() - mapStart
if (mapped.decision.verdict !== protocol.offlineGate.decision ||
    !protocol.offlineGate.requiredFiles.every(file => mapped.files.includes(file)) ||
    mapped.bytes > protocol.offlineGate.maximumBytes) throw new Error("projection offline gate failed")
...
const env = { ...process.env, OPENCODE_CONFIG_CONTENT: JSON.stringify(inline), OPENRELAY_TUI: "off", OPENRELAY_FILTERING: "on",
  OPENRELAY_CONTEXT: "off", OPENRELAY_MEMORY: "off", OPENRELAY_HANDOFF: "off", OPENRELAY_ROUTE: "off", OPENRELAY_ESCALATE: "off",
...
const record = { schemaVersion: 1, label: protocol.label, id, fixture: id, repository: task.repository, model: protocol.model,
  ...
  map: { sha256: sha(mapped.text), bytes: mapped.bytes, files: mapped.files, prepMs: mapPrepMs, graphPrepMs: null },
