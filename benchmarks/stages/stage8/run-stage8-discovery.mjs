...
// One immutable Stage 8 discovery workflow per invocation. No premium calls.
...
const protocolName = process.argv.includes("--protocol-v2") ? "discovery-protocol-v2.json" : "discovery-protocol.json"
const protocolFile = path.join(repositoryRoot, "docs/stage8", protocolName)
...
if (!protocol.tasks.includes(id)) throw new Error("usage: node benchmarks/stages/stage8/run-stage8-discovery.mjs --id <protocol-task-id> [--run]")
...
const prepared = spawnSync("node", [path.join(here, "stage8-tasks.mjs"), "--prepare", id], { cwd: root, encoding: "utf8" })
if (prepared.status !== 0) throw new Error(`prepare failed: ${prepared.stderr}`)
fs.mkdirSync(resultDir, { recursive: true })
const telemetryDir = path.join(defaultRoot(), "data/benchmarks", protocol.label)
const inline = { agent: { "stage8-build": { mode: "primary", maxSteps: 24, description: "Bounded Stage 8 discovery task" } } }
...
function runOpencode() {
  return new Promise(resolve => {
    const child = spawn("opencode", ["run", "-m", protocol.model, "--agent", "stage8-build", "--auto", "--format", "json", fs.readFileSync(promptFile, "utf8").trim()],
      { cwd: workspace, env: launched.env, stdio: ["ignore", "pipe", "pipe"] })
    let stdout = "", stderr = "", timedOut = false
    const timer = setTimeout(() => { timedOut = true; child.kill("SIGTERM"); setTimeout(() => child.kill("SIGKILL"), 5000).unref() }, protocol.limits.timeoutMsPerTask)
...
const v = spawnSync("node", [path.join(here, "stage8-tasks.mjs"), "--verify", id], { cwd: root, encoding: "utf8", timeout: 120000 })
...
record.status = oc.timedOut ? "timeout" : oc.code !== 0 ? "opencode-error" : !record.sessionID ? "missing-session" :
  !record.usageComplete ? "usage-incomplete" : record.premiumCalls > 0 ? "premium-used" : record.verifyPass ? "pass" : "verify-fail"
record.completedAt = new Date().toISOString()
record.totalDurationMs = Date.now() - start
save(path.join(resultDir, "run.json"), record)
