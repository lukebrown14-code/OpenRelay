...
import { buildGraph } from "./map/map.mjs"
import { renderLocatorMap } from "./map/locator.mjs"

const here = path.dirname(fileURLToPath(import.meta.url))
const root = repositoryRoot
const protocolFile = path.join(repositoryRoot, "docs/stage8/locator5-live-protocol.json")
...
if (!protocol.tasks.includes(id)) throw new Error("usage: node benchmarks/stages/stage8/run-stage8-locator5-pilot.mjs --id <protocol-task-id> [--run]")
...
const mapFiles = ["map/map.mjs", "map/python_ast.py", "map/inject.mjs", "map/locator.mjs"].map(f => path.join(here, f))
...
const mapped = renderLocatorMap(graph, fs.readFileSync(promptFile, "utf8"), protocol.map.budgetBytes, protocol.map.maxFiles)
const mapPrepMs = Date.now() - mapStart
const offline = JSON.parse(fs.readFileSync(path.join(repositoryRoot, "docs/stage8/locator5-map-offline-results.json"), "utf8"))
if (offline.summaries.all.fullCoverage < protocol.offlineGate.minimumFullFileCoverage24 ||
    offline.summaries.all.maxBytes > protocol.offlineGate.maximumBytes ||
    (id === "delta-config-writeback" && !protocol.offlineGate.requiredLiveFiles.every(file => mapped.files.includes(file)))) throw new Error("offline map gate failed")
