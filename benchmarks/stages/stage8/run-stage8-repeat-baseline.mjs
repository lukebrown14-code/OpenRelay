...
// One fresh Stage 8 filtering-plus-v3 control run. No map or premium calls.
...
const protocolFile = path.join(repositoryRoot, "docs/stage8/repeat-baseline-protocol.json")
...
if (!protocol.tasks.includes(id)) throw new Error("usage: node benchmarks/stages/stage8/run-stage8-repeat-baseline.mjs --id <protocol-task-id> [--run]")
...
const inline = { agent: { "stage8-build": { mode: "primary", maxSteps: 24, description: "Bounded Stage 8 control repeat" } } }
